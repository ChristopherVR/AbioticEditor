...
///
/// <para>The obvious fix - building as <c>WinExe</c> so no console is allocated at all - cannot
/// be used: the Web SDK only emits <c>wwwroot/_framework</c> (and therefore
/// <c>blazor.web.js</c>) for an <c>Exe</c> output type, so switching leaves the published app
/// opening to a blank window. Hiding the console after startup keeps both.</para>
///
